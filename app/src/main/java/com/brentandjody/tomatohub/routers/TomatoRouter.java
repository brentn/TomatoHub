    ...
    public void prioritize(String ip, long until) {
        try {
            ...
            if (prefs.getBoolean(mContext.getString(R.string.pref_key_allow_changes), false)) {
                ...
                QOSRule outgoingRule = new QOSRule("1<" + ip + "<-1<a<<0<<<<1<" + QOS_COMMENT + ip);
                QOSRule incomingRule = new QOSRule("2<" + ip + "<-1<a<<0<<<<1<" + QOS_COMMENT + ip);
...
class QOSRule {
    enum AddressType {ANY(0), DST_IP(1), SRC_IP(2), SRC_MAC(3);
        private int value;
        private AddressType(int value) {this.value=value;}
    };
